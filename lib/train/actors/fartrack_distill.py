...
class FARTrackDistillActor(BaseActor):
...
#        assert len(data['template_images']) == 5
    ...
    #"""
    # Compute the KL divergence loss between coordinate token features of adjacent layers
    
    # Args:
    #     layer_features (List[torch.Tensor]): 
    #         List of coordinate token features from each layer, each tensor has shape [B,4,D]
    #         Ordered from shallow to deep layers (layer0, layer1, ..., layerN-1)
    
    # Returns:
    #     List[torch.Tensor]: List of KL divergence losses between adjacent layers, with length N-1
    # """
        kl_losses = []
    
        for i in range(len(layer_features) - 1):
            # print("layer_features[i+1]'s shape: ", layer_features[i+1].shape) # layer_features[i+1]'s shape:  torch.Size([4, 32, 605])
            upper_layer = layer_features[i+1].transpose(0, 1).detach()[:, :, :-5]  
            lower_layer = layer_features[i].transpose(0, 1)[:, :, :-5] 
            # print("upper_layer's shape: ", upper_layer.shape) # upper_layer's shape:  torch.Size([32, 4, 600])

            teacher = F.softmax(upper_layer, dim=-1)  
            student = F.log_softmax(lower_layer, dim=-1)   
