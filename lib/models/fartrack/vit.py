...
from lib.models.fartrack.base_backbone import BaseBackbone
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    if pretrained:
        ...
        last_three_block_nums = block_numbers[-3:]  
        ...
        for k, v in checkpoint['model'].items():
            ...
            if k.startswith('blocks.'):
                ...
                if block_num in last_three_block_nums:
                    ...
                    new_checkpoint[new_key] = v.clone()  
...
def _create_dinov3_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
...
def vit_base_patch16_224(pretrained=False, bins=400, range=2, extension=3, **kwargs):
    ...
    # model_kwargs = dict(
    #     patch_size=16, embed_dim=256, depth=12, num_heads=12, bins=bins, range_time=range, extension=extension, **kwargs)
...
def vit_small_patch16_224(pretrained=False, bins=400, range=2, extension=3, **kwargs):
    """
    DinoViT-Small model (ViT-S/16).
    """
    model_kwargs = dict(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, bins=bins, range_time=range, extension=extension, **kwargs)
    model = _create_dinov3_vision_transformer('vit_base_patch16_224_in21k', pretrained=pretrained, **model_kwargs)
    return model
