...
from lib.models.fartrack.vit import vit_base_patch16_224, vit_large_patch16_224, vit_small_patch16_224, vit_tiny_patch16_224
from lib.utils.box_ops import box_xyxy_to_cxcywh


class FARTrack(nn.Module):
...
def build_fartrack(cfg, training=True):
    current_dir = os.path.dirname(os.path.abspath(__file__))  # This is your Project Root
    pretrained_path = os.path.join(current_dir, '../../../pretrained_models')
    if cfg.MODEL.PRETRAIN_FILE and ('FARTrack' not in cfg.MODEL.PRETRAIN_FILE) and training:
        pretrained = os.path.join(pretrained_path, cfg.MODEL.PRETRAIN_FILE)
    else:
        pretrained = ''
    print("pretrained model path: ", pretrained)
