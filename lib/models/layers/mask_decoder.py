...
class MaskDecoder(nn.Module):
	def __init__(self, mask_ratio=0.75, patch_size=16, num_patches=8 ** 2, embed_dim=192, decoder_embed_dim=256,
		...
		self.decoder_embed = nn.Linear(embed_dim, 256, bias=True)

		self.mask_token = nn.Parameter(torch.zeros(1, 1, 256))

		self.decoder_pos_embed = nn.Parameter(torch.zeros(1, num_patches, 256),
		                                      requires_grad=False)  # fixed sin-cos embedding

		self.decoder_blocks = nn.ModuleList([
			Block(256, 8, mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
			for i in range(decoder_depth)])

		self.decoder_norm = norm_layer(256)
		self.decoder_pred = nn.Linear(256, 192, bias=True)  # decoder to patch
